...
namespace GameControl
{
    ...
    public class ObjectPoolManager : Singleton<ObjectPoolManager>
    {
        ...
        ***********************************************************************/
        ...
        [Space] [SerializeField] private bool _testModeOn = true;

        [Space] [SerializeField] private float _poolCleaningInterval = 0.1f; // 풀 한도 초과 오브젝트 제거 간격

        [SerializeField]
        private NonePoolObjectDespawnPolicy _nonePoolObjectDespawnPolicy = NonePoolObjectDespawnPolicy.Destroy;

        [Space] [SerializeField] private List<PoolObjectData> _poolObjectDataList = new List<PoolObjectData>(4);
        ...
        ***********************************************************************/
        ...
        // 디스폰 스케줄링 동기화 큐
        private readonly ConcurrentQueue<CloneScheduleInfo> _despawnScheduleQueue =
            new ConcurrentQueue<CloneScheduleInfo>();
        ...
        ***********************************************************************/
        ...
        private void Test_ChangeContainerName(KeyType key)
        {
            ...
            int cloneCount = _cloneDict.Values.Count(v => v.pool == pool);
        ...
        ***********************************************************************/
        ...
        {
            return _sampleDict.TryGetValue(key, out GameObject sample) ? Instantiate(sample) : null;
        }
        ...
        ***********************************************************************/
        ...
        public void Despawn(GameObject go)
        {
            ...
            DebugLog(_debugDespawn && cloneData.DespawnScheduled,
                $"Despawn 예약 해제 및 즉시 실행 : {go.name}({go.GetInstanceID()})");
        ...
        public void Despawn(GameObject go, float seconds)
        {
            ...
            if (_cloneDict.TryGetValue(go, out CloneScheduleInfo data) == false)
            {
                switch (_nonePoolObjectDespawnPolicy)
                {
                    ...
                    case NonePoolObjectDespawnPolicy.Destroy:
                        DebugLog(_debugDespawn,
                            $"풀에서 제공하지 않은 오브젝트를 {seconds}초 후 파괴합니다 : {go.name}({go.GetInstanceID()})");
                        Destroy(go, seconds);
                        break;
                }
            ...
            DebugLog(_debugDespawn && !data.DespawnScheduled,
                $"{seconds}초 후 Despawn 예약 : {go.name}({go.GetInstanceID()})");
            DebugLog(_debugDespawn && data.DespawnScheduled,
                $"{seconds}초 후 Despawn 예약[갱신] : {go.name}({go.GetInstanceID()})");
