...
namespace TDTK
{
    public class BuildManager : MonoBehaviour
    {
        ...
        public enum _CursorIndicatorMode
        {
            All,
            ValidOnly,
            None
        ...
        public static void HideCursorIndicator()
        {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, maskPlatform))
                {
                    for (int i = 0; i < buildPlatforms.Count; i++)
                    {
                        if (hit.transform == buildPlatforms[i].thisT)
                        {
                            ...
                            Collider[] cols = Physics.OverlapSphere(pos, _gridSize / 2 * 0.9f + footprint * _gridSize,
                                ~maskAll);
                            if (cols.Length > 0)
                            {
        ...
        public static string BuildTower(UnitTower tower)
        {
            ...
            if (suffCost == -1)
            {
                ...
                GameObject towerObj = (GameObject)Instantiate(tower.gameObject, buildInfo.position,
                    buildInfo.platform.thisT.rotation);
        ...
        {
            instance._ShowsampleTowerList(ID);
        }
        ...
        {
            return instance.towerList.Count;
        }
